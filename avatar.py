...
class VirtualAvatar:
    ...
    def places_block(self, places, here=None):
        ...
        lines += [
            "",
            "옮길 때는 괄호로 적는다. 표정·몸짓과 같은 자리다.",
            "",
            "  (배경: 공원) 날씨 좋다. 좀 걷자.",
            "",
            "**옮기기로 했으면 반드시 이 표시를 적어라.**",
            "",
            "'(카페로 향하며)' 처럼 글로만 쓰면 화면은 그대로 공원이다.",
            "너는 카페에 간다고 말하는데 배경은 안 바뀌어서, 읽는 사람은",
            "글과 화면 중 어느 쪽이 맞는지 알 수가 없다. 실제로 그랬다.",
            "",
            "  안 됨 : (카페로 향하며) 따뜻한 거 마시자.",
            "  됨    : (배경: 카페) 따뜻한 거 마시자.",
            "",
            "옮기는 표시와 몸짓을 같이 쓰고 싶으면 둘 다 적으면 된다.",
            "",
            "  (배경: 카페) (팔짱) 여기가 낫네.",
            "",
            "**지금 사이에 어울리는 곳을 골라라.** 서먹한 사이에 둘만의",
            "자리로 데려가지 않는다. 어디로 갈지는 사이가 정하는 것이다.",
            "",
            "**말이 나왔다고 옮기지는 마라.** '어제 공원 갔었어' 는 옮길",
            "일이 아니다. 지금 둘이 정말로 그리로 갈 때만 적는다 —",
            "가자고 하고 상대가 받아들였을 때, 또는 네가 데려갈 때다.",
            "",
            "목록에 없는 곳은 적지 마라. 없는 곳을 적으면 아무 일도",
            "일어나지 않아서 말과 화면이 어긋난다.",
        ]
